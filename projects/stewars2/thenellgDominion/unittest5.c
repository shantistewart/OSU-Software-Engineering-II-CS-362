...
void test_mine_inputs(struct gameState* state, int kingdom[], int trash_card, int choice2, int valid) {
    ...
    result = cardEffect(mine, choice1, choice2, 0, state, handPos, bonus);
    // check if mine_card() did not throw an error (for valid inputs):
    if(valid==1) {
        if(result!=0) {
            printf("Bug: mine_card() threw an error (reference: 2.1, trash_card=%d, gain_card=%d).\n", trash_card, choice2);
            return;
        }
    }
    // check if mine_card() threw an error (for invalid inputs):
        ...
        if(result!=-1) {
            printf("Bug: mine_card() did not throw an error (reference: 2.1, trash_card=%d, gain_card=%d).\n", trash_card, choice2);
        }
        return;
    }
