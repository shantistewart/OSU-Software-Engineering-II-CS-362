...
void test_ambassador_valid_inputs(struct gameState* state, int kingdom[], int numCards) {
    ...
    result = cardEffect(ambassador, hand_index, numCards, 0, state, 0, bonus);
    ...
    if(state->handCount[0] != hand_size0-numCards-1) {
...
// 2. Call ambassador_card() function and assert results:
    ...
    result = cardEffect(ambassador, 0, 3, 0, state, 0, bonus);
    ...
    result = cardEffect(ambassador, 5, 2, 0, state, 5, bonus);
    // check if ambassador_card() threw an error:
    if(result!=-1) {
