...
void test_minion_option1(struct gameState* state, int kingdom[]) {
    ...
    // call minion_card():
    // minion_card(state, 0, choice1, choice2);
    cardEffect(minion, choice1, choice2, 0, state, 0, bonus);
    ...
    // check if Player 0's hand size was decremented by 1 (for discarding current card):
    if(state->handCount[0] != hand_size-1) {
        printf("Bug: Player 0's hand size not decremented properly (reference: 2.1).\n");
    }
}
...
// 2. Call minion_card() function and assert results:
    ...
    // for assignment 5, commented out testing option 2 because it will cause program to get stuck:
